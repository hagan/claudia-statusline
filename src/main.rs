...
fn handle_list_vars(cli: &Cli) -> Result<()> {
    ...
    orchestrator.register(Box::new(stats::StatsProvider::new(
    ...
    orchestrator.register(Box::new(gsd::GsdProvider::new(&full_config.gsd, cwd)));
    ...
    // Directory
    let short_dir = display::Colors::directory()
    ...
    if let Some(name) = input.model.as_ref().and_then(|m| m.display_name.as_deref()) {
        ...
        let model_type = models::ModelType::from_name(&sanitized);
