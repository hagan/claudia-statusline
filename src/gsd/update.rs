...
static UPDATE_CACHE: OnceLock<Mutex<Option<CachedParse<UpdateRecord>>>> = OnceLock::new();
...
pub fn fill_vars(home_dir: &Path, delay_seconds: u64, vars: &mut HashMap<String, String>) {
    ...
    let record = match cache::read_with_cache(&UPDATE_CACHE, &path, parse_update_check) {
        Some(r) => r,
        None => return,
    };
